...
Teijin社のNemLinkサービスからダウンロードしたCPAP概要レポートPDFを、**患者IDごとに指定ページずつ分割し、RS_Baseでの自動ファイリング名にリネームして保存**するWindows用ツールです。

このような1人~~4ページ単位~~ 6ページ単位（2026年6月変更）のレポートです。
...
- PDFを~~4ページ単位~~ 指定ページ単位で分割（各患者1レポート）

  **2026年6月以降、一人あたり6ページになりましたので、分割ページ数を`6`に設定してください。**
- PDFからIDを自動抽出
- RSBaseの自動ファイリング名でファイル名をリネームし保存しますので、あとは`RSBase`のトップ画面リロードや、`RSAuto`で自動ファイリングが可能です。  
  例：`12345~0001~2025_08_01~SAS検査~RSB.pdf`
- 検査名や出力フォルダをGUI上で入力・保存可能
- PDFファイルのドラッグ＆ドロップ対応（`PDFファイル`のテキストボックスにドラッグドロップできます）
- 単一`.exe` 形式で配布・実行可能（PyInstallerでpythonソースから生成しています）
